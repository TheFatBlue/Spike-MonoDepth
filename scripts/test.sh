...
cd "$(dirname "$0")" && python test_DENSE.py \
    --path_to_model /root/autodl-tmp/$model_path/train_s2d_SpikeTransformer/checkpoint-epoch$epoch_num-loss-*.pth.tar \
    --output_path /root/autodl-tmp/$model_path \
    --data_folder /root/autodl-tmp/Spike-Stero/validation
